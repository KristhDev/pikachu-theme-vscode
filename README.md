<h3 align="center">
    <img src="assets/icon.png" width="128" alt="Logo"><br>
    Pikachu Theme for <a href="https://code.visualstudio.com">VSCode</a>
</h3>

<div align="center">

[![Last release](https://img.shields.io/github/v/release/KristhDev/pikachu-theme-vscode?style=for-the-badge&logo=github&logoColor=white&colorA=0d131a&colorB=FAD000)](https://github.com/KristhDev/pikachu-theme-vscode/releases) [![VsCode](https://img.shields.io/badge/vscode-131b24?style=for-the-badge&label=%20&logoColor=BCC3CD&labelColor=0d131a&logo=data:image/svg+xml;base64,)](https://marketplace.visualstudio.com/items?itemName=KristhDev.kristhdev-pikachu-theme-vscode) [![Open VSX](https://img.shields.io/badge/vscode-open%20vsx-131b24?style=for-the-badge&label=%20&logoColor=BCC3CD&labelColor=0d131a&logo=Eclipse%20IDE)](https://open-vsx.org/extension/KristhDev/kristhdev-pikachu-theme-vscode)

</div>

A Visual Studio Code theme inspired by the iconic color palette of Pikachu, the beloved and recognizable mascot of the Pokémon franchise. This theme combines vibrant yellow tones to create a unique and energizing visual experience for developers.

Designed for those seeking a more personalized and nostalgic programming environment, it prioritizes code readability, ensuring that syntactic elements such as keywords, strings, comments, and variables stand out clearly and harmoniously.

Originally, this theme was published by [milab](https://github.com/mIaborde) a few years ago, though I’m unsure why the project was removed. I’ve always been very fond of this theme, so I decided to republish it so other users can enjoy it in Visual Studio Code. Additionally, I’m committed to maintaining it and improving it over time, always respecting the original work and giving proper credit to its author.

## 📸 Previews

<details>
    <summary>HTML</summary>
    <img src="screenshots/html.png">
</details>

<details>
    <summary>CSS</summary>
    <img src="screenshots/css.png">
</details>

<details>
    <summary>JavaScript</summary>
    <img src="screenshots/javascript.png">
</details>

<details>
    <summary>TypeScript</summary>
    <img src="screenshots/typescript.png">
</details>

<details>
    <summary>PHP</summary>
    <img src="screenshots/php.png">
</details>

<details>
    <summary>Python</summary>
    <img src="screenshots/python.png">
</details>

## 📦 Install

- Open the extensions sidebar on Visual Studio Code.

- Search for `Pikachu Theme VsCode` and install it.

- Go to Color Themes settings and choose `Pikachu` as the color theme or `CMD + Shift + P` and enter Command `> color theme` and choose `Pikachu`.

## 🙏 Credits

Thanks to [milab](https://github.com/mIaborde) for the original work.

&nbsp;

<div align="center">

[![License MIT](https://img.shields.io/static/v1.svg?style=for-the-badge&label=License&message=MIT&logoColor=d9e0ee&colorA=0d131a&colorB=FAD000)](https://github.com/KristhDev/pikachu-theme-vscode/blob/main/LICENSE.md)

</div>